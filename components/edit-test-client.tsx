  ...
  useEffect(() => {
    if (initialData?.test && questions.length === 0) {
      ...
      if (initialData.test.scheduledAt) {
        ...
        const d = new Date(initialData.test.scheduledAt);
    ...
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      <aside className="flex w-72 shrink-0 flex-col border-r border-border bg-card">
        <div className="flex items-center gap-2 border-b border-border px-4 py-3">
          <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0" asChild>
            <Link href={`/dashboard/classroom/${classroomId}/tests`}><ArrowLeft className="h-4 w-4" /></Link>
          </Button>
      ...
      <aside className="flex w-80 shrink-0 flex-col border-l border-border bg-card overflow-y-auto">
        ...
        <div className="p-4 border-t border-border">
          <Button onClick={handleSave} disabled={isPending || isLoading} className="w-full h-11 text-sm font-semibold">
            {isPending ? "Updating Test…" : "Save Changes"}
          </Button>
        </div>
      </aside>
    </div>
