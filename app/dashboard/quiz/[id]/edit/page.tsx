...
import { useUpdateQuiz } from "@/hooks/tanstackQuery/quiz/use-update-quiz";
...
export default function EditQuizPage() {
  ...
  const { data: quizResponse, isLoading: isLoadingQuiz } = useGetQuiz(params.id);
  const { mutate: updateQuiz, isPending: isUpdating } = useUpdateQuiz(params.id);
  ...
  useEffect(() => {
    if (quizResponse?.quiz && !hasLoaded.current) {
      setQuizTitle(quizResponse.quiz.title || "");
      setQuizDescription(quizResponse.quiz.description || "");
      if (quizResponse.quiz.questions?.length > 0) {
        setQuestions(
          quizResponse.quiz.questions.map((q: any) => ({
            id: q.id,
            type: q.type,
            text: q.text || "",
        ...
        setQuestions([defaultQuestion("single_choice")]);
      }
  ...
  const handleSave = () => {
    ...
    updateQuiz(
      { quizId: Number(params.id), title: quizTitle, description: quizDescription, questions: formatted },
      {
        ...
        onError: (err: Error) => {
          toast.error(err.message || "Failed to update quiz. Please check all fields.");
        },
      }
    );
  };
