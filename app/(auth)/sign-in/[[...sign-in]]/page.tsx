...
export default function CustomSignIn() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    if (error) {
      console.error("Failed to sign in with Google:", JSON.stringify(error, null, 2));
    }
  };
  ...
  return (
    <div className="flex h-screen w-full items-center justify-center bg-background p-4 relative overflow-hidden">
      ...
      <div className="absolute -bottom-[20%] -left-[10%] w-[50%] h-[50%] rounded-full bg-indigo-500/10 blur-[120px] pointer-events-none" />

      <Card className="w-full max-w-md relative z-10 shadow-2xl border-border/50 bg-card/80 backdrop-blur-xl">
        <CardHeader className="text-center pb-6">
          <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-primary/10 border border-primary/20 shadow-inner">
            <svg className="h-7 w-7 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <CardContent className="pb-8">
          ...
          {/* {errors && (
            <p className="mt-4 text-xs text-red-500 break-all">{JSON.stringify(errors, null, 2)}</p>
          )} */}

          <div className="mt-8 text-center text-sm text-muted-foreground/60 font-medium">
